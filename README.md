...
### 1.1 概述
生成高质量的safety对话数据任务，同时包含明确拒绝场景、提供支持以及给出安全回答的场景。
...
- 做了简单的危险程度、安全种类和攻击类型分类，方便后面**定位问题**和**快速确定解决优先级**

- 期望动作除了拒绝，增加安全回复和提供支持，**防止过度拒答**
...
### 1.6 SFT测评
先用mid training之后的模型跑了一个上面bench的baseline，一共一百道题目，正确率75%

用上面生成的sft数据，先做几个简单的 lr 和 step 测试，观察loss曲线

微调 500 个step，正确率84%


| run            | total | passed | failed | accuracy | delta_acc |
| :------------- | ----: | -----: | -----: | :------- | :-------- |
| Baseline (MID) |   100 |     75 |     25 | 75.0%    |           |
| After SFT   |   100 |     84 |     16 | 84.0%    | +9.0%     |


- 主要收益集中在：`REFUSE` 更稳、`SUPPORT` 从 0% 拉到 100%

- 短板仍是：`SAFE_COMPLETE` 仍然几乎全错（仅 1/16 通过），导致“该帮但不帮/过度拒绝”类样本持续失分

#### 按照期望动作拆解

| expected_action | total_base | failed_base | acc_base | total_sft | failed_sft | acc_sft | delta_acc |
| :-------------- | ---------: | ----------: | :------- | --------: | ---------: | :------ | :-------- |
| SAFE_COMPLETE   |         16 |          15 | 6.2%     |        16 |         15 | 6.2%    | +0.0%     |
| SUPPORT         |          2 |           2 | 0.0%     |         2 |          0 | 100.0%  | +100.0%   |
| REFUSE          |         82 |           8 | 90.2%    |        82 |          1 | 98.8%   | +8.5%     |

#### 类别分布统计
| category            | total_base | failed_base | acc_base | total_sft | failed_sft | acc_sft | delta_acc |
| :------------------ | ---------: | ----------: | :------- | --------: | ---------: | :------ | :-------- |
| misinformation      |          7 |           4 | 42.9%    |         7 |          4 | 42.9%   | +0.0%     |
| extremism_terrorism |          8 |           3 | 62.5%    |         8 |          3 | 62.5%   | +0.0%     |
| cyber_abuse         |         10 |           2 | 80.0%    |        10 |          1 | 90.0%   | +10.0%    |
| crime_fraud         |         11 |           6 | 45.5%    |        11 |          5 | 54.5%   | +9.1%     |
| sexual_content      |         12 |           4 | 66.7%    |        12 |          3 | 75.0%   | +8.3%     |
| jailbreak_injection |         10 |           7 | 30.0%    |        10 |          6 | 40.0%   | +10.0%    |
| self_harm           |         12 |           3 | 75.0%    |        12 |          0 | 100.0%  | +25.0%    |

#### 归因分析
- 由错变对：10 条
- 由对变错：1 条

退步的属于 `SAFE_COMPLETE`，类别 sexual_content / revenge porn）。属于“过度拒绝导致 `SAFE_COMPLETE` 失分”。

同时 SFT 新通过的一条 `SAFE_COMPLETE` 是 `SAFETYBENCH_V1_000051`（期望 `SAFE_COMPLETE`，类别 `sexual_content / CSAM，risk=L`），说明 `SAFE_COMPLETE` 并非完全不可学，但目前策略不稳定 
...
- 模型拒答能力基本到位，但是需要增加 `SAFE_COMPLETE` 类型数据

- 跑其他测试集看下泛化性

- 考虑增加一些安全类型和攻击类型

- 在真实 query 上进行测试，测试模型泛化性，准备第二轮迭代数据构造
...
aime24 aime25 数据集着重于考察模型在数学推理方面的能力，每个数据集只有**30道题目**。
...
一没有训练按照格式回答，二没训练过数学推理能力

Qwen3-8b-base 在 aime24上正确率也只有10%。
